    ...
    <div className="space-y-4">
      {config.sections.map((section) => (
        <div key={section.title} className="rounded-xl border border-white/10 bg-black/20 p-4">
          <h4 className="mb-3 text-sm font-semibold text-white/90">{section.title}</h4>
          <div className="grid gap-3 md:grid-cols-2">
            {section.fields.map((field) => {
              ...
              if (field.type === 'select') {
                return (
                  <label key={field.key} className="flex flex-col gap-1 text-sm md:col-span-1">
                    <span className="text-white/70">{field.label}</span>
                    <select
                      value={value}
                      onChange={(event) => onChange(field.key, event.target.value)}
                      className="rounded-lg border border-white/20 bg-black/40 px-3 py-2 outline-none transition focus:border-neon"
                    >
              ...
              if (field.type === 'textarea') {
                return (
                  <label key={field.key} className="flex flex-col gap-1 text-sm md:col-span-2">
                    <span className="text-white/70">{field.label}</span>
                    <textarea
                      value={value}
                      onChange={(event) => onChange(field.key, event.target.value)}
