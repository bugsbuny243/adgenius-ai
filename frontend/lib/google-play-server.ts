...
export class GooglePlayPublisherProvider {
  ...
  async publishRelease(input: PublishInput): Promise<PublishResult> {
    ...
    try {
      ...
      return { status: 'failed', errorMessage: toActionableMessage(error, 'Service account geçersiz.') };
    }
    ...
    try {
      ...
      const uploadResp = await fetch(
        `https://androidpublisher.googleapis.com/upload/androidpublisher/v3/applications/${input.packageName}/edits/${editId}/bundles?uploadType=media`,
        {
          method: 'POST',
          headers: {
          ...
          releases: [
            {
              ...
              releaseNotes: [{ language: 'tr-TR', text: input.releaseNotes || 'Game Factory yayın güncellemesi' }]
            }
          ]
        })
      });
