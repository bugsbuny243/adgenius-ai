  ...
  return (
    <main>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <article className="panel"><h2 className="text-xl font-semibold">Entegrasyon Özeti</h2><div className="mt-3 space-y-2 text-sm text-white/80"><p><span className="text-white/60">Google:</span> {googleConnection?.status === 'active' ? 'Bağlı' : 'Bağlı değil'}</p><p><span className="text-white/60">YouTube channel:</span> {typeof googleMeta.youtubeChannelId === 'string' ? googleMeta.youtubeChannelId : '—'}</p><p><span className="text-white/60">Blogger blog:</span> {typeof googleMeta.bloggerBlogId === 'string' ? googleMeta.bloggerBlogId : '—'}</p><p><span className="text-white/60">Son güncelleme:</span> {googleConnection?.updated_at ? new Date(googleConnection.updated_at).toLocaleString('tr-TR') : '—'}</p></div><div className="mt-4 flex flex-wrap gap-2"><Link href="/connections" className="inline-flex rounded-lg border border-white/20 px-3 py-1.5 text-sm hover:border-neon">Bağlantıları yönet</Link><Link href="/settings/integrations/google-play" className="inline-flex rounded-lg border border-white/20 px-3 py-1.5 text-sm hover:border-neon">Google Play bağlantıları</Link></div></article>
