...
import { type GameBrief, generateGameBriefWithAi } from '@/lib/ai-engine';
...
export async function POST(request: Request) {
  try {
    ...
    const gameBrief = await generateGameBriefWithAi(prompt);
    ...
    const { error: insertError } = await serviceRole.from('game_briefs').insert({
      ...
      prompt,
      model: process.env.KOSCHEI_MODEL?.trim() || "meta-llama/Meta-Llama-3.1-70B-Instruct",
