...
export function ChatThread({ messages, loading }: ChatThreadProps) {
  return (
    <section className="rounded-2xl border border-zinc-800 bg-zinc-950/60 p-4">
      <h2 className="mb-3 text-sm font-medium text-zinc-200">Çalıştırma Akışı</h2>
      ...
      {!loading && messages.length === 0 ? (
        <p className="rounded-xl border border-dashed border-zinc-700 bg-zinc-900/40 p-4 text-sm text-zinc-300">
          Bir görev yazarak başlayın. Görev girdisi ve üretilen sonuç burada akış halinde görünür.
        </p>
      ) : null}
      ...
      <div className="max-h-[440px] space-y-3 overflow-y-auto pr-1">
        {messages.map((message) => {
          ...
          return (
            ...
            >
              <p className="mb-1 text-xs uppercase tracking-wide text-zinc-400">{isUser ? 'Görev' : 'Sonuç'}</p>
